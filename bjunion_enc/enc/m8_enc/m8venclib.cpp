...
#define QP_MIN 22
#define QP_MAX 34//36
#define INTRA_QP_MIN 22//20
#define INTRA_QP_MAX 34//32
...
inline void fill_p_buffer_nv21(m8venc_drv_t *para, unsigned char *input_addr, int mb_x, int mb_y, unsigned char *ref_mb, bool multi_mv)
{
    ...
    short *temp = NULL;
    int mb_index = para->src.mb_width * mb_y + mb_x;
    mvx = para->mb_buffer[mb_index].me.mv_info.average_ref.x ;
    mvy = para->mb_buffer[mb_index].me.mv_info.average_ref.y;
...
static unsigned calc_target_qp(unsigned cur_bits, unsigned cur_qp, unsigned target_bits)
{
    unsigned ret_qp = 51;
    unsigned qp = 51;
    float ratio = 0.0;
        ...
        for (mb_x = 0; mb_x < p->src.mb_width; mb_x++)
        {
            ...
            get_intra_range(p, mb_x, mb_y, &intra_min, &intra_max);
            {
                ...
                if (((pred_bits + p->mb_buffer[index].ie.coeff_bits) <= mb_target) || (p->mb_buffer[index].ie.coeff_bits == 0))
                    ...
                    qp = calc_target_qp((unsigned)p->mb_buffer[index].ie.coeff_bits, p->base_quant, ie_coeff_bits);
    ...
    for (mb_y = 0; mb_y <  p->src.mb_height; mb_y++)
    {
        for (mb_x = 0; mb_x < p->src.mb_width; mb_x++)
        {
            ...
            if ((mb_y != 0) || (mb_x != 0))
            {
                ...
                if ((p->mb_buffer[index].ie.coeff_bits < p->mb_buffer[index].me.coeff_bits) && (intra_bits < p->mb_buffer[index].me.mb_cost))
                {
                    intra_mode = true;
                }
            }
...
RE_CHECK:
            ...
            if (coeff_bits > 0)
            {
                ...
                if (qp_change == true)
                {
                    ...
                    {
                        qp = calc_target_qp(p->mb_statics.mb_predict, p->base_quant, mb_target);
                    }
                }
                else
                {
...
void *InitM8VEncode(int fd, amvenc_initpara_t *init_para)
{
    ...
    p->ref_info.y = NULL;
    p->ref_info.uv = NULL;
    p->control.half_pixel_mode = true;
    p->control.fix_qp = -1;
    p->control.multi_mv = false;
    p->control.mv_correction = true;
