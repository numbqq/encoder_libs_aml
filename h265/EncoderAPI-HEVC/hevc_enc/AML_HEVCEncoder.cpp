...
AMVEnc_Status Wave4VpuInit(AMVHEVCEncHandle *Handle, Uint32 coreIdx) {
    ...
    VLOG(INFO, "\nvdi_get_common_memory, pady:0x%08lx\n", vb.phys_addr);
...
AMVEnc_Status Wave4VpuCreateInstance(AMVHEVCEncHandle *Handle, int alloc) {
    ...
    VLOG(INFO, "W4_ADDR_WORK_BASE %x\n", temp);
    temp = VpuReadReg(Handle->instance_id, W4_WORK_SIZE);
    VLOG(INFO, "W4_WORK_SIZE %x\n", temp);
    return ret;
}
...
AMVEnc_Status Wave4VpuEncSeqInit(AMVHEVCEncHandle *Handle, int alloc) {
    ...
    VLOG(INFO, "Wave4VpuEnCSeqInit- start\n");
    // instance information
    if (alloc) {
...
AMVEnc_Status Wave4VpuEncSeqSet(AMVHEVCEncHandle *Handle) {
    ...
    rd_ptr = VpuReadReg(Handle->instance_id, W4_BS_RD_PTR);
    wr_ptr = VpuReadReg(Handle->instance_id, W4_BS_WR_PTR);
    VLOG(INFO, "Wave4VpuEnCSeqSet, rd:0x%x, wr:0x%x\n", rd_ptr, wr_ptr);
...
AMVEnc_Status AML_HEVCSetInput(AMVHEVCEncHandle *Handle, AMVHEVCEncFrameIO *input) {
    ...
    if (Handle->bitrate != input->bitrate) {
        ...
        VLOG(INFO, "HEVC set bitrate to %d", Handle->bitrate);
    ...
    if ((input->scale_width !=0 && input->scale_height !=0) || input->crop_left != 0 ||
        ...
        if (Handle->enc_width % 32) {
            for (int i = 0; i < Handle->enc_height; i++) {
                memcpy(y + i * luma_stride, (void *) ((char *) ge2dinfo.dst_info.vaddr + i * Handle->enc_width), Handle->enc_width);
            }
        } else {
            memcpy((void *)Handle->src_vb[Handle->src_idx].virt_addr, ge2dinfo.dst_info.vaddr,  Handle->enc_width * Handle->enc_height);
        }
        y = (char *) (Handle->src_vb[Handle->src_idx].virt_addr + size_src_luma);
        memcpy(y, (void *) ((char *)ge2dinfo.dst_info.vaddr + Handle->enc_width * Handle->enc_height), chroma_stride * Handle->enc_height / 2);
    } else
#endif
    {
        // set frame buffers
