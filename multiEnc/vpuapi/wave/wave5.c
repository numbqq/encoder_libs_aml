...
RetCode Wave5VpuEncGiveCommand(CodecInst *pCodecInst, CodecCommand cmd, void *param)
{
    ...
    VLOG(ERR, "Wave5VpuEncGiveCommand inst %p cmd 0x%x param %p\n",
...
RetCode Wave5VpuDecRegisterFramebuffer(CodecInst* inst, FrameBuffer* fbArr, TiledMapType mapType, Uint32 count)
{
    ...
    if (mapType == COMPRESSED_FRAME_MAP || mapType == COMPRESSED_FRAME_MAP_SVAC_SVC_BL) {
        ...
        if (inst->codecMode == W_HEVC_DEC){
            ...
            fbcYTblSize        = WAVE5_FBC_LUMA_TABLE_SIZE(initPicWidth, initPicHeight);    // FIX ME
        } 
        else {
            /* Unknown codec */
            return RETCODE_NOT_SUPPORTED_FEATURE;
        ...
        if (inst->codecMode == W_HEVC_DEC) {
            ...
            fbcCTblSize        = WAVE5_FBC_CHROMA_TABLE_SIZE(initPicWidth, initPicHeight);  // FIX ME
        } 
        else {
            /* Unknown codec */
            return RETCODE_NOT_SUPPORTED_FEATURE;
...
RetCode Wave5VpuDecFlush(CodecInst* instance, FramebufferIndex* framebufferIndexes, Uint32 size)
{
    ...
    VLOG(ERR, "VWave5VpuDecFlush inst %p FramebufferIndex %p size = %d\n",
        instance, framebufferIndexes, size);
...
RetCode Wave5VpuSleepWake(Uint32 coreIdx, int iSleepWake, const Uint16* code, Uint32 size, BOOL reset)
{
    ...
    VLOG(INFO, "Wave5VpuSleepWake iSleepWake %d, code %p, size %d, reset %d \n",
...
RetCode Wave5VpuDecSetBitstreamFlag(CodecInst* instance, BOOL running, BOOL eos, BOOL explictEnd)
{
    ...
    VLOG(ERR, "Wave5VpuDecSetBitstreamFlag inst %p running %d, eos %d explictEnd %d\n",
                instance, running, eos, explictEnd);
...
Int32 Wave5VpuWaitInterrupt(CodecInst* instance, Int32 timeout, BOOL pending)
{
    ...
    VLOG(INFO, "Wave5VpuWaitInterrupt instanc %p, timeout %d, pending %d\n",
...
RetCode Wave5VpuBuildUpEncParam(CodecInst* instance, EncOpenParam* param)
{
    ...
    if (instance->productId == PRODUCT_ID_525)
        pEncInfo->vbWork.size       = WAVE525ENC_WORKBUF_SIZE;
    else if (instance->productId == PRODUCT_ID_520)
        pEncInfo->vbWork.size       = WAVE520ENC_WORKBUF_SIZE;
    else if (instance->productId == PRODUCT_ID_521)
...
RetCode Wave5VpuEncInitSeq(CodecInst* instance)
{
    ...
    */
    ...
    VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_AUTO_FRM_SKIP_DROP, pOpenParam->vbvThreshold<<8 | pOpenParam->qpThreshold<<2 | pOpenParam->enAutoFrmDrop<<1 | pOpenParam->enAutoFrmSkip);  
#endif
    ...
    VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_INTRA_PARAM, (pParam->decodingRefreshType<<0) | (pParam->intraQP<<3) | (pParam->intraPeriod<<16));
    ...
    VpuWriteReg(coreIdx, W5_CMD_ENC_SEQ_RC_TARGET_RATE_BL, pOpenParam->bitRateBL);
...
RetCode Wave5VpuEncGetSeqInfo(CodecInst* instance, EncInitialInfo* info)
{
    ...
    VpuReadReg(instance->coreIdx, W5_RET_DONE_INSTANCE_INFO);
