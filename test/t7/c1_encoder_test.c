...
int main(int argc, char *argv[])
{
	...
	if (result_avc_1 == NULL || result_avc_2 == NULL) {
        ...
        //h.264 test 1
        system("aml_enc_test /vendor/bin/nv21_480p.yuv  /data/c1_es.h264.1 640 480 120 30 800000 1 2 0 2 4");

	sleep(1);
        //h.264 test 2
        system("aml_enc_test /vendor/bin/nv21_480p.yuv  /data/c1_es.h264.2 640 480 120 30 800000 1 2 0 2 4");

        if (my_system("md5sum /data/c1_es.h264.1", result_avc_1, RET_LEN) != 0) {
            printf("md5sum /data/c1_es.h264.1 failed\n");
            goto EXIT;
	}
        if (my_system("md5sum /data/c1_es.h264.2", result_avc_2, RET_LEN) != 0) {
            printf("md5sum /data/c1_es.h264.2 failed\n");
            goto EXIT;
	}
	...
	if (result_hevc_1 == NULL || result_hevc_2 == NULL) {
        ...
        //h.265 test 1
        system("aml_enc_test /vendor/bin/nv21_480p.yuv  /data/c1_es.h265.1 640 480 120 30 800000 1 2 0 2 5");

	sleep(1);

        //h.265 test 2
        system("aml_enc_test /vendor/bin/nv21_480p.yuv  /data/c1_es.h265.2 640 480 120 30 800000 1 2 0 2 5");

        if (my_system("md5sum /data/c1_es.h265.1", result_hevc_1, RET_LEN) != 0) {
            printf("md5sum /data/c1_es.h265.1 failed\n");
            goto EXIT;
	}
        if (my_system("md5sum /data/c1_es.h265.2", result_hevc_2, RET_LEN) != 0) {
            printf("md5sum /data/c1_es.h265.2 failed\n");
            goto EXIT;
	}
