...
int compute_vmaf(double *vmaf_score, char *fmt, int width, int height,
                 ...
                 int do_ms_ssim, char *pool_method, int n_thread, int n_subsample,
...
enum ScoreAggregateMethod { MEAN, HARMONIC_MEAN, MINIMUM };
...
{
  public:
    virtual Result run(Asset asset,
                       int (*read_frame)(float *ref_data, float *main_data, float *temp_data,
                                         int stride, void *user_data),
                       void *user_data, bool disable_clip, bool enable_transform, bool do_psnr,
                       bool do_ssim, bool do_ms_ssim, int n_thread, int n_subsample) = 0;
...
{
  public:
    static std::unique_ptr<IVmafQualityRunner> createVmafQualityRunner(const char *model_path,
